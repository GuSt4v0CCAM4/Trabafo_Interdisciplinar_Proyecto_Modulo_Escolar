...
<html>
	...
	$sub="";
		if((isset($_POST["sem"])) && (isset($_POST["dateFrom"])) && (isset($_POST["dateTo"])) ){
			...
			$from = $_POST["dateFrom"];
			 $to = $_POST["dateTo"];
			 $final = $_POST["dateTo"];
			//Viendo si los valores se postearon adecuadamente o no
			if($sem=="" || $from=="" || $to=""){
			...
			//Si todo está bien, se inicia con la tarea
				if($sem =="I-I"){
					...
					while($row = mysqli_fetch_array($sql1)){
							...
							//Contando el numero total de dias basados en los id registrados en total de todos los cursos
				...
				$sql3 = mysqli_query($connect, "SELECT count(atten) AS LOSS FROM `a1` WHERE `id`='$ids' and atten = '1' and day between '$from' and '$final' ");
							if($rows = mysqli_fetch_array($sql3)){
								//Recuperando el numero de dias de ausencia
								$totalall1 = $rows["LOSS"];
							}
						
							//cContando los dias totales para un id en particular
							 $totalall = $totalall1+$totalall2;
							
						
							//Contando el numero total de dias presente de acuerdo a curso
	$sql2 = mysqli_query($connect, "SELECT count(atten) AS ATTEN FROM `a1` WHERE (`id`='$ids' and `fac` = '$sub' and atten = '1' and day between '$from' and '$final') ");
							while($rows = mysqli_fetch_array($sql2)){
								//Recuperando numero de dias presente
								 $total1 = $rows["ATTEN"];
